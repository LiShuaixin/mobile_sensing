...
public:
    ...
    inline Eigen::Matrix4f get_pose() 
    {
    
        Eigen::Matrix4f T = Eigen::Matrix4f::Identity();
        T.topLeftCorner<3, 3>() = Rwc_;
        T.topRightCorner<3, 1>() = twc_;
	...
	PointCloud tmp;
	std::list<size_t>::const_iterator iter_mask = ground_point_idx_.begin();
    ...
    inline PointCloud::Ptr get_ground_cloud()
    {
	// we donot erase point from pointcloud, since the erasing of point is too slow, 
	// which is more than 100 times slower than create a new pointcloud pointer
	PointCloud tmp;
...
public:       
    ...
    std::list<size_t> ground_point_idx_;
    std::unordered_map<uint16_t, std::list<size_t>> clusters_point_idx; // first = label index; second = point index
    std::vector<size_t> corner_points_less_sharp_;
    std::vector<size_t> corner_points_sharp_; 
    std::vector<size_t> surface_points_less_flat_;
    std::vector<size_t> surface_points_flat_;
